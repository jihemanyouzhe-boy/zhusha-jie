...
### v2.1（2026-08-20 · 10 项新功能 + 整体 UI 美化）

**功能一 · 今日修行计划**
- 新增「今日计划」独立导航页，展示每日五课（晨诵/省思/运动/善行/早眠）及完成进度，点击即可切换功课状态，五课圆满额外奖励功德 +8

**功能二 · 修行统计面板**
- 新增「修行统计」导航页，近 30 日打卡热力图（30 格小方块），汇总功德/连续日/累计日/番茄钟/呼吸数/拒念/答题/破戒等全部修行指标，带修行评语

**功能三 · 经典名句收藏**
- 经典诵读每句均可收藏/取消收藏，收藏按钮实时状态切换
- 新增「经典收藏」导航页，展示全部收藏名句，支持取消收藏

**功能四 · 打卡里程碑**
- 连续打卡达到 7/21/30/100/365 日时额外奖励功德（+20/30/50/100/365），打卡庆祝浮层提示里程碑达成

**功能五 · 打卡补签**
- 近 7 天漏签补打卡（功德 +3），每日限补一次，已打卡日不可再补
- 打卡页面底部显示近 7 天日期按钮，按日补签

**功能六 · 日记全文搜索**
- 日记列表新增搜索框，按关键词搜索日记内容，后端 SQL LIKE 查询

**功能七 · 树洞全文搜索**
- 树洞心声列表新增搜索框，按关键词搜索全站心声

**功能八 · 共勉墙全文搜索**
- 共勉墙列表新增搜索框，按关键词搜索勉语内容

**功能九 · 统一 UI 美化**
- 新增搜索框组件（.input.search-box 圆角胶囊样式，focus 展开动画）
- 新增 mini-heat 热力图样式（30 格小色块）
- 树洞/共勉标题区增加 flex-wrap，搜索框与排序标签同行布局
- 补签按钮组 flex-wrap 布局，标签已打卡状态加 done 样式
- 经典诵读每句增加收藏按钮（`.liked` 状态保留）
- 主题按钮悬停动效更协调，小屏适配优化

**功能十 · 后端 10 个新 API**
- `GET /plan` — 每日修行计划
- `GET /stats` — 修行统计（近 30 天打卡序列 + 总览指标）
- `GET /classic/fav` — 收藏列表
- `POST /classic/fav` — 收藏/取消收藏（toggle）
- `POST /classic/fav/delete` — 删除收藏
- `POST /checkin/backfill` — 补签打卡
- `GET /journal?q=` — 日记搜索
- `GET /treehole?q=` — 树洞搜索
- `GET /encourage?q=` — 共勉搜索
- 打卡接口新增 `milestone` / `milestoneBonus` 返回值
